...
const processorEndpoint =
  "https://us-documentai.googleapis.com/v1/projects/500843166981/locations/us/processors/9a89a0ae110dcf9e:process";
  ...
  try {
    ...
    if (!imageUrl || !userId) {
      return NextResponse.json(
        { message: "Image URL and userId are required" },
        { status: 400 },
      );
    }
    ...
    if (!imageResponse.ok) {
      ...
      return NextResponse.json(
        { message: "Failed to fetch image", status: imageResponse.status },
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { message: "Document AI processing failed", error },
    ...
    const labeledFile = labeledBucket.file(
      `labeled_${new Date().toISOString()}.json`,
    );
...
function processDocumentAIResult(document: any): Menu {
  ...
  const menu: Menu = {
    items: document.entities.map((entity: any) => ({
      ...
      price: parseFloat(
        entity.properties.find((prop: any) => prop.type === "price")
          ?.mentionText || "0",
      ),
      description: entity.properties.find(
        (prop: any) => prop.type === "description",
      )?.mentionText,
    })),
  };
  return menu;
