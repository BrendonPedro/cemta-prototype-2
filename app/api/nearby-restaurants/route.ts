...
import { getCachedRestaurantsForLocation, saveCachedRestaurantsForLocation, getCachedImageUrl, saveImageUrlCache } from '@/app/services/firebaseFirestore';
import { uploadImageToBucket, getImageFromBucket } from '@/app/services/gcpBucketStorage';
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const nearbyRestaurants = await Promise.all(response.data.results.slice(0, parseInt(limit)).map(async (place) => {
      let photoUrl = null;
      if (place.place_id && place.photos && place.photos.length > 0) {
        const photoReference = place.photos[0].photo_reference;
        
        // Check if we have a cached image URL
     let cachedImageUrl = await getCachedImageUrl(place.place_id, place.name ?? 'unknown');
        ...
        if (cachedImageUrl) {
          ...
          try {
            const imageResponse = await axios.get(googlePhotoUrl, { responseType: 'arraybuffer' });
            const contentType = imageResponse.headers['content-type'];
            const imageBuffer = Buffer.from(imageResponse.data, 'binary');

            // Upload to GCP bucket
            if (place.place_id) {
              photoUrl = await uploadImageToBucket(place.place_id, imageBuffer, contentType);

              // Save URL to Firestore cache
              await saveImageUrlCache(place.place_id, place.name ?? 'unknown', photoUrl);
            }
          } catch (error) {
            console.error(`Failed to fetch or cache photo for ${place.name}:`, error);
            photoUrl = null;
          }
        }
      }
