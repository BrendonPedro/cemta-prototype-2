...
export async function POST(req: NextRequest) {
  try {
    ...
    const { menuData, menuId, restaurantId, menuName, imageUrl } =
      await req.json();
    ...
    // Extract restaurantName from menuData
    const restaurantName = menuData?.restaurant_info?.name?.original || menuName || 'Unknown';
    ...
    return NextResponse.json(
      { message: "Menu data saved successfully", menuId },
      { status: 200 },
    );
  } catch (error: any) {
