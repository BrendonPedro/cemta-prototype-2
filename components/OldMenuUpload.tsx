...
const OldMenuUpload = () => {
  ...
  const [view, setView] = useState<"documentAI" | "vertexAI">("documentAI");
  const [documentAiResult, setDocumentAiResult] = useState<Menu | null>(null);
  ...
  const handleUploadAndProcess = async () => {
    ...
    try {
      ...
      const documentAiResponse = await fetch("/api/process-document-ai", {
        method: "POST",
        headers: {
      ...
      const { menu } = await documentAiResponse.json();
      setDocumentAiResult(menu);
      await saveDocumentAiResults(userCredential.user.uid, url, menu);

      // Process with Vertex AI
      await handleVertexAiProcess(url, idToken);
    } catch (error) {
  ...
  const handleVertexAiProcess = async (imageUrl: string, idToken: string) => {
    try {
      const vertexAiResponse = await fetch("/api/vertex-ai", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${idToken}`,
      ...
      const { ocrText, processingId } = await vertexAiResponse.json();
      setVertexAiResult(ocrText);
      setLatestProcessingId(processingId);
      await saveVertexAiResults(userId as string, ocrText);
    } catch (error) {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      ...
      <div className="mt-6">
        <Button onClick={() => setView("documentAI")} className="mr-2">
          View Document AI Results
        </Button>
        <Button onClick={() => setView("vertexAI")}>
          View Vertex AI Results
        </Button>
      </div>
      ...
      {view === "documentAI" && documentAiResult && (
        <DocumentAiResultsDisplay userId={userId as string} />
      )}
      {view === "vertexAI" && vertexAiResult && (
